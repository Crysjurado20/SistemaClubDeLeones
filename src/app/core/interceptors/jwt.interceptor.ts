...
const SKIP_FORBIDDEN_REDIRECT_HEADER = 'X-Skip-Forbidden-Redirect';
...
export const jwtInterceptor: HttpInterceptorFn = (
    ...
    const skipForbiddenRedirect = req.headers.has(SKIP_FORBIDDEN_REDIRECT_HEADER);
    const cleanReq = skipForbiddenRedirect
        ? req.clone({ headers: req.headers.delete(SKIP_FORBIDDEN_REDIRECT_HEADER) })
        : req;

    const request =
        token && !shouldSkipAuthHeader(cleanReq)
    ...
    return next(request).pipe(
        catchError((err: unknown) => {
            if (err instanceof HttpErrorResponse) {
                ...
                if (err.status === 403) {
                    console.warn(
                        `[JWT Interceptor] 403 en ${request.url}. skipForbiddenRedirect=${skipForbiddenRedirect}`,
                    );
